...
public class DeferredMessageBuilder {
    ...
    public OMElement getDocument(MessageContext msgCtx, InputStream in) throws
		...
		if (transportHeaders != null) {
			...
			if (contentType.equals(org.apache.synapse.transport.netty.passthru.Constants.DEFAULT_CONTENT_TYPE)
					&& (contentLength == null || Integer.valueOf(contentLength) == 0)
					&& trasferEncoded == null) {
				msgCtx.setProperty(org.apache.synapse.transport.netty.passthru.Constants.NO_ENTITY_BODY, true);
				msgCtx.setProperty(Constants.Configuration.CONTENT_TYPE, "");
				msgCtx.setProperty(org.apache.synapse.transport.netty.passthru.Constants.RELAY_EARLY_BUILD, true);
    ...
    public static String getContentType(String contentType, MessageContext msgContext) {
        ...
        if (null != msgContext.getProperty(org.apache.synapse.transport.netty.passthru.Constants.INVOKED_REST)
                && msgContext.getProperty(org.apache.synapse.transport.netty.passthru.Constants.INVOKED_REST).equals(true)
